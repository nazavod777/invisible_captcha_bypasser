...
class ReCaptcha:
    ...
    @staticmethod
    async def get_recaptcha_token(endpoint: str,
                                  params: str,
                                  client: AsyncSession) -> str:
        r: Response = await client.get(url=f'https://www.google.com/recaptcha/{endpoint}/anchor?{params}')

        results: list = re.findall(r'"recaptcha-token" value="(.*?)"', r.text)

        return results[0]

    @staticmethod
    async def get_recaptcha_response(endpoint: str, params: str,
                                     data: str,
                                     client: AsyncSession) -> str:
        r: Response = await client.post(url=f'https://www.google.com/recaptcha/{endpoint}/reload?{params}',
                                        data=data,
                                        headers={
                                            "Content-Type": "application/x-www-form-urlencoded"
                                        })

        results: list = re.findall(r'"rresp","(.*?)"', r.text)
    ...
    async def solve_recaptcha(cls,
        ...
        async with AsyncSession() as client:
            ...
            params: dict = dict(pair.split('=') for pair in data['params'].split('&'))

            post_data: str = POST_DATA.format(params["v"],
                                              token,
                                              params["k"],
                                              params["co"])

            captcha_response: str = await cls.get_recaptcha_response(endpoint=data['endpoint'],
                                                                     params=f'k={params["k"]}',
                                                                     data=post_data,
                                                                     client=client)
